...
import styleIconCss from './style/icon.css'
...
export default {
  init({
    ...
    if (inline) {
      this._entryBtn.hide()
      this._$el.addClass('roderuda-inline')
      this.show()
    }
  },
  _isInit: false,
  version: VERSION,
  util: {
    isRodErudaEl: util.isRodErudaEl,
    evalCss,
    isDarkTheme(theme) {
      if (!theme) {
        theme = this.getTheme()
  ...
  _checkInit() {
    if (!this._isInit) logger.error('Please call "roderuda.init()" first')
    return this._isInit
  },
  _initContainer(container, useShadowDom) {
    ...
    extend(el, {
      className: 'roderuda-container __chobitsu-hide__',
  ...
  _initStyle() {
    const className = 'roderuda-style-container'
    const $el = this._$el
